...
class DefaultDispatcher(RecordDispatcher):
    def dispatch_response(self, record, env):
        ...
        ipaddr = handler.record.get_header(b'IP-address')
    ...
    def dispatch_revisit(self, record, env):
        ...
        if record.get_header(b'WARC-Profile') and record.get_header(
                b'WARC-Profile').endswith(b'/revisit/server-not-modified'):
            return None
        if record.ip_address == b'127.0.0.1':
            return None
    ...
    def dispatch_resource(self, record, env):
        ...
        if record.url.startswith(b'ftp://'):
            return FtpHandler
        elif record.url.startswith((b'http://', b'https://')):
