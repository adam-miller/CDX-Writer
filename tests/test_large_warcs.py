    ...
    with open(cdx, 'rb') as f, open(hashcdx, 'wb') as w:
        for l in f:
            if not l.startswith(b' '):
                urlkey, ts, original, rest = l.split(b' ', 3)
                urlkey = md5(urlkey).hexdigest().encode('ascii')
                original = md5(original).hexdigest().encode('ascii')
                l = b' '.join([urlkey, ts, original, rest])
            w.write(l)
